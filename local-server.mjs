...
http.createServer(async (req, res) => {
  ...
  const id = randomUUID(); const input = join(workspace, `${id}-input.mp4`); const result = join(output, `${basename(filename, ".mp4")}-${target.replace(":", "x")}-enhanced.mp4`);
  try {
...
}).listen(4173, "127.0.0.1", () => console.log("Vivid local server: http://127.0.0.1:4173"));
